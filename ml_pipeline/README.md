...
- Logs antigos continuam legíveis, mas não é possível corrigir retroativamente o atraso entre imagem e comando. Eles aparecem como `legacy_unknown` na proveniência. `--require-aligned` aceita histórico de comandos válido e a coleta `teacher_same_frame` do professor. Para o professor, exige decisão posterior à captura, idade inferior a 200 ms e consistência dos timestamps com tolerância de 1 ms. Método, instante/idade da decisão e confiança original são preservados em `sources_*.csv`. Recoletar é preferível quando os logs antigos não comprovam alinhamento.
